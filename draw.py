...
def draw_polygons( points, screen, color ):
    v = [0,0,-1]
    for p in range(0, len(points), 3): #take every third point
        a = [points[p+1][0] - points[p][0],
            points[p+1][1] - points[p][1],
            points[p+1][2] - points[p][2] ]
        b = [points[p+2][0] - points[p][0],
            points[p+2][1] - points[p][1],
            points[p+2][2] - points[p][2] ]
        normal = [a[1]*b[2] - a[2]*b[1],
                a[2]*b[0] - a[0]*b[2],
                a[0]*b[1] - a[1]*b[0] ]
        if((normal[0]*v[0] + normal[1]*v[1] + normal[2]*v[2]) < 0): #is within viewing range
...
def add_sphere( points, cx, cy, cz, r, step ):
    ...
    while lat < lat_stop:
        longt = 0
        add_polygon(points, temp[0][0], temp[0][1], temp[0][2], 
                    temp[(lat - 1) * num_steps + 1][0], temp[(lat - 1) * num_steps + 1][1], temp[(lat - 1) * num_steps + 1][2],
                    temp[lat * num_steps + 1][0], temp[lat * num_steps + 1][1], temp[lat * num_steps + 1][2] )
        while longt < longt_stop:
            previouscirclepoint = (lat - 1) * num_steps + longt
            index = lat * num_steps + longt            
            if (longt < longt_stop/2 ):
                add_polygon(points, temp[previouscirclepoint+1][0], temp[previouscirclepoint+1][1], temp[previouscirclepoint+1][2], 
            ...
            if (longt == longt_stop/2):
                ...
                temp[index-1][0], temp[index-1][1], temp[index-1][2],
                temp[previouscirclepoint-1][0], temp[previouscirclepoint-1][1], temp[previouscirclepoint-1][2])
...
def add_torus( points, cx, cy, cz, r0, r1, step ):
    ...
    while lat < lat_stop:
        longt = 0
        tempindex = lat * num_steps + longt
        while longt < longt_stop - 1:
            
            index = lat * num_steps + longt
            previouscirclepoint = (lat - 1) * num_steps + longt
            #connect the points on each circle to form solid circles
            #add_edge( points, temp[index][0], temp[index][1], temp[index][2], 
            #        temp[index+1][0], temp[index+1][1], temp[index+1][2] )
            #Add the lateral lines
            #add_edge( points, temp[index][0], temp[index][1], temp[index][2], 
            #        temp[previouscirclepoint][0], temp[previouscirclepoint][1], temp[previouscirclepoint][2])
            add_polygon(points, temp[index][0], temp[index][1], temp[index][2], 
                        temp[index+1][0], temp[index+1][1], temp[index+1][2],
                        temp[previouscirclepoint+1][0], temp[previouscirclepoint+1][1], temp[previouscirclepoint+1][2])
            longt+= 1
        #edge case for last (1/step) arc of every circle
        #add_edge( points, temp[index][0], temp[index][1], temp[index][2], 
        #        temp[tempindex][0], temp[tempindex][1], temp[tempindex][2] )
        #
        #edge cases aren't so bad.
        index = lat * num_steps + longt
        previouscirclepoint = (lat - 1) * num_steps + longt
        add_polygon(points, temp[index][0], temp[index][1], temp[index][2],
                    temp[tempindex][0], temp[tempindex][1], temp[tempindex][2], 
                    temp[(lat - 1) * num_steps][0], temp[(lat - 1) * num_steps][1], temp[(lat - 1) * num_steps][2])
        lat+= 1
